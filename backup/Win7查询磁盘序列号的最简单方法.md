前言：今天公司新入职一个女同事，我帮忙给她重装电脑系统，公司有现成的PE启动盘，所以很丝滑地就装好了。市场部的电脑很旧了，只能装win7系统凑合用用。按照公司规定，装完系统还要记录一下设备的内网IP、MAC地址以及磁盘序列号。前两个不成问题，一个`ipconfig/all`命令解决，没想到查个磁盘序列号倒难住我了，颇费一番周折，所以浅浅记录一下。
