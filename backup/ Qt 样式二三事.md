...
这不是一句抽象原则，而要直接落地，变成三条约定：

1. 样式只在应用启动时加载一次
2. 控件差异通过属性表达，不继承、不写分支
3. Qt Designer 只负责布局

在项目中严格遵守约定，样式问题就不会失控。
...
因为 qt 项目可在多个位置设置样式，容易造成样式间的冲突、覆盖等，不符合预期的情况。
针对样式的最佳实践，应当是：
1. 用 qApp->setStyleSheet("...") 在 main() 里就先设好全局或父控件样式
2. 附带 ui 文件的类，会用 ui->setupUi(this);   再加载 Designer 中的样式
3. 然后再在之后的代码中针对单个控件微调（如果需要），类似于 ui->toolButton_close->setStyleSheet("...");
运行时会输出 QSS 语法错误到控制台（debug 模式下看 Application Output）
按照样式
